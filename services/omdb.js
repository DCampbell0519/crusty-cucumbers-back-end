...
async function searchByTitle(title, year) {
  ...
  if (title && title.trim().length >= 2) {
    const url = `${BASE_URL}?apikey=${API_KEY}&type=movie&s=${encodeURIComponent(title)}${year ? `&y=${year}` : ''}&page=1`
  ...
  for (const seed of seeds) {
    const url = `${BASE_URL}?apikey=${API_KEY}&type=movie&s=${encodeURIComponent(seed)}${year ? `&y=${year}` : ''}&page=1`
    const { data } = await axios.get(url)
